...
Client-side routing allows you to provide a faster surfing experience on the browser. This type of routing uses JavaScript to maintain the state or memory of your application so that the page does not have to reload when a new URL is requested.

Client-side routing allows you to load smaller portions of the webpage as opposed to requesting/loading the entire thing with server-side routing. This means the site is faster and uses less bandwitch because it is not reloading the same unnecessary data like the header and footer.
